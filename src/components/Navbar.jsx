...
function Navbar() {
  const navLink = ["Services", "Our Work", "About Us", "Insights", "Contact"];
  const [visibleSidebar, setVisibleSidebar] = useState(false);
  return (
    <div className="relative">
      <div className="fixed z-[999] w-full px-6 lg:px-20 py-8 flex items-center justify-between">
        ...
        <div className="lg:flex gap-10 hidden">
          {navLink.map((item, index) => (
            <a
              key={index}
              className={`text-md capitalize  cursor-pointerfont-light ${
                index === 4 && "ml-32"
              }`}
            >
              {item}
      ...
      <div
        className={`fixed top-0 w-full pt-32 h-screen bg-zinc-900 duration-200  z-30 lg:hidden ${
          visibleSidebar ? "trasnlate-y-0" : "-translate-y-full"
        }`}
      >
      <div className="border-t-[1px] border-zinc-500 px-6 font-sans pt-5">
        {navLink.map((item, index) => (
          <h2 key={index} className="text-6xl cursor-pointer mb-2 last:mb-0">{item}</h2>
        ))}
      </div>
      </div>
    </div>
